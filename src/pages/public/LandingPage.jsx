...
import { ArrowRight, Briefcase, CheckCircle2, MapPin, Search, ShieldCheck, Star, Users, Zap } from 'lucide-react';
...
export default function LandingPage() {
  ...
  const featuredCompanies = [...companies].sort((a, b) => b.rating - a.rating).slice(0, 3);
  ...
  return (
    <div>
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:flex lg:items-center lg:gap-12 lg:px-8 lg:py-28">
          <div className="lg:w-1/2">
            ...
            <div className="mt-10 grid grid-cols-3 gap-6 border-t border-ink-200/60 pt-6">
              {HERO_STATS.map((s) => (
                <div key={s.key}>
                  <p className="text-2xl font-bold text-ink-900">{counts[s.key]}</p>
          ...
          <div className="mt-12 lg:mt-0 lg:w-1/2">
            <div className="card relative mx-auto max-w-md p-6">
              ...
              <p className="text-sm font-semibold text-ink-500">Latest opportunity</p>
              {openJobs[0] && (
                <>
                  <h3 className="mt-1 text-lg font-bold text-ink-900">{openJobs[0].title}</h3>
                  ...
                  <p className="mt-3 line-clamp-3 text-sm text-ink-600">{openJobs[0].description}</p>
                  <div className="mt-4 flex items-center justify-between">
                    <p className="text-lg font-bold text-brand-600">
                      ${openJobs[0].paymentAmount.toLocaleString('en-CA')}
                      <span className="ml-1 text-xs font-medium text-ink-500">
                        {openJobs[0].paymentType === 'hourly' ? '/ hour' : 'fixed'}
                      </span>
                    </p>
                    <Link to={`/jobs/${openJobs[0].id}`} className="btn-primary text-xs">
                      View job
                    </Link>
                  </div>
                </>
      ...
      <section className="bg-white py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-2xl text-center">
            <h2 className="text-3xl font-bold tracking-tight text-ink-900">Built for both sides</h2>
            <p className="mt-3 text-ink-600">
              Whether you're staffing a tower fit-out or looking for your next gig, Jobnet gives you the
              tools to move quickly and confidently.
            </p>
          </div>
          ...
          <div className="mt-12 grid gap-6 md:grid-cols-3">
            {[
              ...
              <div key={f.title} className="card p-6 transition-shadow hover:shadow-cardHover">
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-ink-500">{company?.name}</p>
                    </div>
      ...
      <section className="bg-white py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold tracking-tight text-ink-900">Top-rated companies</h2>
          <p className="mt-1 text-sm text-ink-500">Verified employers known for great work environments.</p>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {featuredCompanies.map((c) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h3 className="text-base font-bold text-ink-900">{c.name}</h3>
                ...
                <p className="mt-2 line-clamp-2 text-xs text-ink-600">{c.description}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section className="bg-ink-900 py-16">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <p className="mt-3 text-ink-300">
            Join Jobnet today as a worker or an employer. It only takes a minute.
          </p>
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <Link to="/register?role=worker" className="btn-primary px-5 py-3 text-base">
              I'm a worker
            </Link>
            <Link
