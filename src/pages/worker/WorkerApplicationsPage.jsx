...
export default function WorkerApplicationsPage() {
	...
	const filtered =
		tab === "all" ? myApps : myApps.filter((a) => a.status === tab);
			...
			<div className='flex flex-wrap items-center gap-2 border-b border-ink-200'>
				{TABS.map((t) => {
					const count =
						t === "all"
							? myApps.length
							: myApps.filter((a) => a.status === t).length;
					return (
						<button
							key={t}
							type='button'
						...
						return (
							<div key={a.id} className='card p-5'>
									...
									<div>
										...
										{company && (
											<Link
												...
												className='ml-2 text-sm text-ink-500 hover:underline'>
												- {company.name}
											</Link>
										)}
										<div className='mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-ink-500'>
											...
											<span className='inline-flex items-center gap-1'>
												...
												{new Date(job.dueDate).toLocaleDateString("en-CA")}
											</span>
											<span>
												Applied{" "}
								...
								<div className='mt-3 flex flex-wrap items-center justify-end gap-2'>
									...
									{a.status === "selected" &&
										company &&
										!hasReviewedCompany(company.id, job.id) && (
											<button
												type='button'
