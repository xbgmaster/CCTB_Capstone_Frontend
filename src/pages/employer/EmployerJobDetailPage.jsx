...
export default function EmployerJobDetailPage() {
	...
	const reload = useCallback(async () => {
		...
		try {
			...
			(apps || []).forEach((a) => {
				ensureWorkerProfile(a.workerId);
			});
		} catch (err) {
			console.error(err);
			toast({
	...
	if (!job && !loading) {
		return (
			<div className='card p-6 text-center'>
				...
				<Link to='/employer/jobs' className='btn-primary mt-3 inline-flex'>
					Back to jobs
				</Link>
			</div>
		);
	...
	const setStatus = async (app, status) => {
		try {
			...
			toast({
				type: "success",
				title: `${app.workerFirstName || "Applicant"} marked as ${status}`,
			});
	...
	return (
		<div className='space-y-4'>
			...
			<div className='flex flex-wrap items-center gap-2 border-b border-ink-200'>
				{TABS.map((t) => {
					const count =
						t === "all"
							? jobApps.length
							: jobApps.filter((a) => a.status === t).length;
					return (
						<button
							key={t}
							type='button'
			...
			{loading ? (
				...
				<div className='space-y-3'>
					{filteredApps.map((a) => {
						...
						const initials = (
							(a.workerFirstName?.[0] || "?") + (a.workerLastName?.[0] || "?")
						).toUpperCase();
						return (
			...
			<Modal
				...
				title={
					reviewing ? `Review ${reviewing.workerFirstName || "worker"}` : ""
				}
				footer={
					<div className='flex justify-end gap-2'>
						...
						<button type='submit' form='review-form' className='btn-primary'>
							Submit review
						</button>
					</div>
				}>
